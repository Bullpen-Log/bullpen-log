  ...
  return (
    ...
    >
      ...
      <g className="stroke-ink" strokeWidth="2" strokeLinecap="round" fill="none">
        <circle cx="120" cy="40" r="7" className="fill-ink/20" />
        <line x1="120" y1="47" x2="118" y2="76" />
        {/* 던지는 팔 */}
        <line x1="119" y1="54" x2="140" y2="42" />
        <line x1="140" y1="42" x2="158" y2="34" className="stroke-sky" />
        {/* 글러브 팔 */}
...
export function FilmingGuide({ defaultOpen = false }: { defaultOpen?: boolean }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-line bg-surface">
      ...
      >
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-sky-soft/50 bg-sky/10 text-sky">
        ...
        <ChevronDown
          className={`h-5 w-5 shrink-0 text-muted transition-transform ${open ? 'rotate-180 text-sky' : ''}`}
        />
      </button>
          ...
          <ul className="space-y-3">
            {CHECKLIST.map((item) => (
              <li key={item.title} className="flex gap-3">
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-sky-soft/50 bg-sky/10 text-sky">
