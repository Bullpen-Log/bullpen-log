...
const RULES = [
  ...
  [
    /스플릿 ?스쿼트|불가리안|런지|스텝 ?업|스텝업|피스톨|시시|스케이터|사이드 ?스텝|몬스터 ?워크|스타 ?스텝|워킹/,
    '런지',
  ],
  ...
  // 않게 뒤에 오는 말까지 함께 본다.
  [
    /레터럴 ?(미니 ?)?허들|레터럴 ?미디얼|레터럴 ?셔플|레터럴 ?바운드|레터럴 ?홉|레터럴 ?점프|레터럴 ?드라이브|측면 ?홉|레터럴 ?원판|스플릿 ?스탠스|스케이트/,
    '런지',
  ],
  // ── 힌지 (고관절 지배)
  [
    /데드리프트|RDL|힌지|굿모닝|굿 ?모닝|힙 ?쓰러스트|힙 ?스러스트|브리지|스윙|풀 ?스루|햄스트링|노르딕|킥백|힙 ?익스텐션|백 ?익스텐션|스내치|클린|스냅다운|글루트[·ㆍ・]?햄|글루트 ?햄/,
    '힌지',
  ],
  // ── 스쿼트 (양측 무릎 지배)
  [
    /스쿼트|레그 ?프레스|박스 ?점프|점프 ?스쿼트|뎁스 ?드롭|포고|홉|바운드|브로드 ?점프|월 ?싯|드롭[- ]?캐치/,
    '스쿼트',
  ],
  /*
  ...
  // ── 밀기
  [
    /프레스|푸쉬업|푸시업|팔굽혀|딥스|딥 |익스텐션|푸시 ?아웃|푸쉬 ?아웃|푸시다운|푸쉬다운|체스트 ?패스/,
    '밀기',
  ],
  // ── 당기기
  // '컬'만 쓰면 '버티(컬) 점프'가 당기기로 들어온다 — 컬은 이름을 붙여 좁힌다
  [
    /로우|풀업|친업|풀 ?다운|풀다운|이두 ?컬|해머 ?컬|덤벨 ?컬|바벨 ?컬|프리처|페이스 ?풀|풀 ?어파트|풀 ?오버|밴드 ?풀|슈러그|랫 |당기기|데드행|행잉/,
    '당기기',
  ],
  // ── 회전 (몸통을 돌리거나, 돌아가지 않게 버티거나)
  [
    /회전|로테이|초핑|촙|찹|리프트|팔로프|파로프|비틀|트위스트|스로우|던지|토스|슬램|디스로테|안티 ?로테|사이드 ?플랭크|러시안|180도/,
    '회전',
  ],
  // ── 운반 (들고 버티거나 걷는다)
