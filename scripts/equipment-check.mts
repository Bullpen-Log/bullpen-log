 ...
 *
 * 맨몸만 가진 사람은 할 수 있는 운동이 넷에 하나꼴이다. 그런 경우에도 훈련이
 * 통째로 비지 않는지, 시간이 얼마나 짧아지는지, 무엇 하나만 사면 가장 크게
 * 늘어나는지를 실제 등록된 운동으로 확인한다.
 *
 * 암케어는 여기서 보지 않는다 — 2026-09-25 모든 일정에서 빠져 암케어 화면에서
 * 따로 한다.
 *
...
for (const { label, owned } of PROFILES) {
  ...
  for (const theme of THEMES) {
    ...
    /* 운동이 하나도 안 나온 날을 문제로 본다 — 장비가 적다고 훈련이 없어지면 안 된다 */
