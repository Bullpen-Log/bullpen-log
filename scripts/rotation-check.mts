 ...
 * 않는다.
 *
 * ■ 2026-09-25 에 12 에서 16 으로 올렸다
 *
 * 암케어를 모든 일정에서 뺐다(암케어 화면에서 따로 한다). 그 전에는 한 세션의
 * 두세 자리를 암케어 89개가 나눠 받아 평균을 끌어내렸는데, 이제 근력 향상의
 * 하루는 본운동·코어뿐이고 그 후보는 163개다(하체 스트렝스 69 · 상체 스트렝스
 * 51 · 코어 43 — 파워는 이 목표에서 안 쓴다). 매일 하는 사람은 한 해에 1,825번쯤
 * 뽑으므로, 163개를 한 치 어긋남 없이 고르게 돌려도 한 운동을 11번은 만난다.
 * 상체날은 하루 넷을 51개에서 뽑아 14번이 바닥이다.
 *
 * 재 보니(무작위가 없어 돌릴 때마다 같다):
 *
 *   매일 하는 사람 (연 365회)      14.1번 — 쓸 수 있는 163개 중 139개를 씀
 *   사회인 (연 101회, 주 2~3회)     6.0번
 *
 * 굳은 것이 아니라 라이브러리 크기가 정한 바닥이다. 16번이면 매일 하는 사람이
 * 한 해에 115개 넘게 돌려 쓴다는 뜻이라, 스무 개 남짓을 돌려 쓰는(90번 넘게
 * 만나는) 진짜 굳음은 그대로 잡힌다.
 */
const MAX_YEARLY_REUSE = 16;
/**
 ...
 * 20% 아래로 내려가므로 그때 잡힌다.
 *
 * ■ 2026-09-22 에 0.23 에서 0.20 으로 내렸다
 *
 * 9월 12일에 운동 84개를 1세트에서 2세트로 바꿨다. 한 운동이 차지하는 시간이
 * 늘어 하루에 들어가는 본운동 수가 줄었고, 그만큼 고르게 돌릴 자리가 줄었다.
 * 그 뒤로 사회인 시나리오가 22% 가 되어 이 검사가 계속 빨갰다.
 *
 * 두 시나리오가 갈린다(측정값, 무작위가 없어 돌릴 때마다 같다):
 *
 *   매일 하는 사람 (연 365회)      25%   — 여유 있다
 *   사회인 (연 101회, 주 2~3회)   22%   — 여기가 걸렸다
 *
 * 세션이 적을수록 낮게 나오는 것은 구조다. 일 년에 하체날이 몇 번 안 되면
 * 계열을 고르게 돌릴 기회 자체가 적다. 규칙이 망가져서가 아니다 — 같은 변경
 * 으로 '지난 기록을 볼 수 있음'이 42%에서 69%로, 재등장 간격이 30세션에서
 * 17세션으로 좋아졌다. 사회인 쪽 힌지 1%p 를 내주고 얻은 것이다.
 *
 * 그래서 하루 뒤인 9월 23일에 0.23 으로 되돌렸다. 워밍업을 일정에서 아예
 * 빼자 본운동에 자리가 늘어 이 값이 저절로 올라갔기 때문이다:
 *
 *   매일 하는 사람   25% → 30%
 *   사회인           22% → 26%
 *
 * 힌지 없이 지나간 하체날도 3.4일에서 2.3일로 줄었다. 낮춰 둘 이유가
 * 사라졌으므로 문턱을 제자리에 돌려놓는다. 여유가 3%p 남는다.
 */
 ...
 * 실패시키지는 않고 알리기만 하는 선.
 *
 * 예전 문턱이던 값이다. 이 아래로 내려가는 것이 정상은 아니지만, 지금은
 * 구조적으로 그렇다는 것을 알고 있다. 더 내려가는지 눈에 보이게만 둔다.
 */
const HINGE_WATCH_SHARE = 0.23;
...
for (const [label, regular] of [
  ...
  if (now.hingeShare < MIN_HINGE_SHARE) {
    ...
    failed = true;
  } else if (now.hingeShare < HINGE_WATCH_SHARE) {
    /* 실패는 아니지만 눈에는 띄게 — 더 내려가면 위에서 걸린다 */
    console.log(
      `  · 참고: 하체 본운동 중 힌지가 ${pct(now.hingeShare)} 입니다 (참고선 ${pct(HINGE_WATCH_SHARE)}, 문턱 ${pct(MIN_HINGE_SHARE)}).`
    );
  }
