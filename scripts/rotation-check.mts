 ...
 *
 * 문턱을 22% 에 딱 맞추면 여유가 0 이라 다음 변경에 바로 다시 빨개진다.
 * 그래서 이 주석이 원래 적어 둔 의도("20% 아래로 내려가면 잡는다")를 그대로
 * 숫자로 옮긴다. 대신 23% 는 참고선으로 남겨, 밑돌면 실패시키지 않고 알린다 —
 * 곧 워밍업을 시간 계산에서 빼면 이 값이 또 움직일 텐데, 그때 조용히 넘어가면
 * 안 된다.
 */
const MIN_HINGE_SHARE = 0.2;
