...
- 김민 쪽 파일을 고친 것:
  ...
  - '홈의 오늘 체크인'으로 가던 링크(트레이닝 화면 멈춤 카드, 암케어 멈춤, AI 맞춤의 통증 의심,
    ...
    `/pitch-log/<오늘>` 로 보낸다.
- 검토하고 고친 것(뒤 커밋):
  - `app/(app)/pitch-log/[date]/page.tsx` · `day-client.tsx` — 홈의 투구 상자가 하던 '계획보다 많이
    던졌다' 알림을 오늘 날짜 화면으로 옮겼다(`todayLimits` prop). 저장을 막지는 않는다.
  - `app/(app)/today/pitch-log-panel.tsx` — 체크인을 새로 남긴 날은 받아 둔 하루치 상세를 버리고
    다시 읽는다(종에서 체크인을 고쳐도 달력 밑 요약이 옛 값으로 남던 것).
  - `components/app-shell.tsx` — '오늘 안 던졌어요' 저장을 창이 아니라 AppNav 가 쥔다(두 번 눌러 둘이
    생기던 것), 종을 열 때 `router.refresh()`, 화면 전환 연출 중에 막대의 단추를 누르면 대신 눌러 준다
    (`run()` 안의 click 리스너 — 판이 열릴 때는 넘기지 않는다), 다른 창이 뜨면 알림 창을 닫는다.
