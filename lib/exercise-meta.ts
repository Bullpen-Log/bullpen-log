 ...
 *
 * 쓰는 운동이 하나도 없는 이름은 두지 않는다. 목록에 있어도 후보를 넓힐 뿐
 * 좁히지는 않아 해가 없지만, 가진 장비를 고르는 화면에서 "이걸 켜면 뭐가
 * 나오나" 하고 켜 봐도 아무것도 안 늘어난다 — 고를 것이 없는 칸이 남는다.
 * 야구공·폼롤러·마사지볼·머신을 그래서 뺐다(2026-09-01).
 */
...
export const MOVEMENT_PATTERNS = [
   ...
   * 종아리를 따로 둔다.
   *
   * 무릎을 굽히는 것도 고관절을 접는 것도 아니라서 스쿼트·힌지 어디에도 못
   * 넣는다. 그렇다고 비워 두었더니 겹침 판정을 아예 안 받아서, 하체 근력 날
   * 본운동의 32%가 카프 레이즈로 채워졌다 — 하루에 네 개가 나온 날도 있었다.
   * 따로 두면 다른 계열과 똑같이 '하루에 하나'로 걸린다.
   */
  { name: '카프', desc: '발목으로 밀어낸다 — 카프 레이즈' },
] as const;
...
export const DRILL_EQUIPMENT = [
   ...
   * 물이 든 도구.
   *
   * 짐볼로 등록돼 있었는데 둘은 다른 물건이다. 짐볼은 공기가 든 큰 공이고,
   * 이쪽은 물이 출렁여 무게 중심이 계속 움직인다 — 그 흔들림을 버티는 것이
   * 이 드릴의 내용이라, 짐볼로 대신할 수가 없다.
   */
  '워터백',
  '워터볼',
] as const;
